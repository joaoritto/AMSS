...
function IM_FOC1(Labor,Z,λold,Z_grid)
...
function IM_λ(Labor,Z,λold,Z_grid)
...
function IM_B(Labor,Z,λold,Z_grid)
...
function IM_nZ(Labor,Z,λold,Z_grid)
...
function IM_Payoff(Labor,Z,Vold,Z_grid)
...
function VfunctionIteration(Z_grid;V0=false,Labor0=false)


    ssizeZ=size(Z_grid)[1]
    Vold=zeros(ssizeZ)
    λold=zeros(ssizeZ)
    ...
    Vnew=zeros(ssizeZ)
    λnew=zeros(ssizeZ)
    Labor_sol=zeros(ssizeZ,4)
    B_sol=zeros(ssizeZ)
    nZ_sol=zeros(ssizeZ,4)
    ϵ=1e-10
    error=1000
    iter=0
